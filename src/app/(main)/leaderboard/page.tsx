...
export default function LeaderboardPage() {
    ...
    const rankedUsers = useMemo((): RankedUser[] => {
        ...
        const usersWithStats = game.accounts
            .filter(account => !account.player.isBanned)
            .map(account => {
    ...
    return (
        <div className="container mx-auto px-4 py-6">
            <h1 className="font-headline text-3xl md:text-4xl font-bold mb-4 flex items-center gap-3"><Trophy className="text-yellow-400 shrink-0"/> Leaderboard</h1>
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead className="text-right w-[100px]">Sync %</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                        ...
                                        <TableRow key={rankedUser.player.username} className={cn(isCurrentUser && 'bg-primary/10 hover:bg-primary/20')}>
                                            ...
                                            <TableCell>
                                                <Link href={`/users/${rankedUser.player.username}`} className="flex items-center gap-3 group min-w-0">
                                                    <GamifiedAvatar account={userAccount} imageClassName="w-8 h-8 md:w-10 md:h-10 shrink-0" />
                                                    <div className="min-w-0">
                                                        <p className="font-bold text-[10px] md:text-sm group-hover:text-primary transition-colors truncate max-w-[100px] md:max-w-[200px]">{rankedUser.player.displayName}</p>
                                                         {displayTitle && <Badge variant="destructive" className="mt-0.5 text-[7px] md:text-[9px] py-0 px-1 truncate max-w-[80px] md:max-w-none">{displayTitle}</Badge>}
                                                    </div>
                                                </Link>
                                            </TableCell>
                                            <TableCell className="text-right font-mono text-[10px] md:text-lg text-primary">{rankedUser.totalScore.toLocaleString()}</TableCell>
                                            <TableCell className="text-right font-mono text-[10px] md:text-lg">{rankedUser.completionPercentage.toFixed(1)}%</TableCell>
                                        </TableRow>
                                    );
                                })}
                            </TableBody>
