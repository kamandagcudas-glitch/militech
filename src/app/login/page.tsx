...
export default function LoginPage() {
  ...
  const handleGuestLogin = async () => {
    const result = await game.loginAsGuest();
    if (!result.success) {
      toast({
        variant: "destructive",
        title: "Guest Access Failed",
        description: result.message || "Could not initiate guest session.",
      });
    }
  };
      ...
      <Card className="w-full max-w-[95vw] sm:max-w-sm shadow-2xl bg-card/80 backdrop-blur-sm border border-primary/20 z-10 animate-in fade-in zoom-in-95 duration-500">
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button variant="outline" className="h-11 text-[10px] uppercase tracking-widest font-mono border-primary/10 hover:bg-primary/5" onClick={handleGoogleLogin}>
            ...
            <Button variant="outline" className="h-11 text-[10px] uppercase tracking-widest font-mono border-primary/10 hover:bg-primary/5" onClick={handleGuestLogin}>
