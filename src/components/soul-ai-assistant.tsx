...
export default function SoulAiAssistant() {
  ...
  const { data: historyData, isLoading: isHistoryLoading } = useDoc<{ messages: AiMessage[] }>(historyDocRef);
  ...
  const handleSend = async () => {
    ...
    if (historyDocRef) {
      setDoc(historyDocRef, { messages: newMessages }, { merge: true });
    }
    ...
    try {
      ...
      if (historyDocRef) {
        setDoc(historyDocRef, { messages: finalMessages }, { merge: true });
      }
    } catch (error) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      {!isOpen ? (
        ...
        <Card className="w-[380px] h-[550px] flex flex-col shadow-2xl border-primary/30 animate-in slide-in-from-bottom-4 duration-300">
              ...
              <div className="space-y-4">
                {messages.length === 0 && (
                  <div className="text-center py-12 text-muted-foreground">
                    ...
                    <p className="font-cyber text-xs">Soul Initialized. How can I help, Admin?</p>
                  </div>
                )}
                {messages.map((msg, i) => (
                ...
                {isTyping && (
                  <div className="flex gap-3">
                    ...
                    <div className="bg-muted p-3 rounded-lg border border-white/5">
                      <p className="text-xs italic text-muted-foreground animate-pulse">Soul is thinking of something witty...</p>
                    </div>
                  </div>
                )}
                <div ref={scrollRef} />
